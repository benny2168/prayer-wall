...
export default function NewChainForm({ organizations }: { organizations: any[] }) {
  const [loading, setLoading] = useState(false);

  const handleFileSizeCheck = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <form 
      action={async (formData) => {
        setLoading(true);
        try {
          // The action itself handles redirect on success
          await createPrayerChain(formData);
        } finally {
          setLoading(false);
        }
      }} 
    ...
    >
      ...
      <div className="pt-6 flex justify-end items-center space-x-4 border-t border-[--color-border-base]">
        ...
        <button type="submit" className="btn-primary" disabled={organizations.length === 0 || loading}>
          {loading ? "Deploying..." : "Deploy Prayer Chain"}
        </button>
      </div>
    </form>
  );
